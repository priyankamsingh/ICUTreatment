...
class PerformanceTracker:
    def __init__(self):
        ...
        self.tracking_performance_dict = {
            ...
            'model_WDR': [],
            'model_wis': [] ,
            'dqn_WDR': [], #changed here
            'dqn_wis': [] , #changed here
    ...
    def track_append(self, iteration, eval_type, model_WDR, model_wis, dqn_WDR, dqn_wis, best_action_prob):
        ...
        self.tracking_performance_dict['dqn_WDR'].append(dqn_WDR)
        self.tracking_performance_dict['dqn_wis'].append(dqn_wis)
...
def track_performance(data_dict, i, gamma, model, pt, pi_behavior, MDP_Q, eval_type, use_gpu):        
        ...
        DQN_Q = pd.DataFrame.from_records(outputs)
        DQN_Q = np.around(DQN_Q,3)
        DQN_Q.columns =  ['Q' + str(i) for i in np.unique(data_dict[eval_type]['action'])]

        #############################################
        # Perform WDR analysis
        model_WDR, model_wis = eval_WDR(data_dict, eval_type, gamma, pi_evaluation, pi_behavior, MDP_Q)
        dqn_WDR, dqn_wis = eval_WDR(data_dict, eval_type, gamma, pi_evaluation, pi_behavior, DQN_Q) # adding DQN_Q # added it PS
        ...
        # Track results
        print( str(eval_type) + "\nDQN WDR_: " + str(round(dqn_WDR, 3)) 
                              + "\nDQN WIS_: " + str(round(dqn_wis, 3)))
        
        # track 
        pt.track_append(i, eval_type, model_WDR, model_wis, dqn_WDR, dqn_wis, best_action_prob)
        print('-' * 10)
